...
import time
from .audio_processor import process_audio_file, extract_mel_spectrogram

class AudioCommandDetector:
    def __init__(self, model_path='../models/audio_classifier_best.pth'):
        # Khởi tạo model
    ...
    def _preprocess_audio(self, audio_data, sr):
        ...
        # Find 1.5s segment with highest energy
        ...
        energy = np.array([np.sum(audio_data[i:i+window_size]**2) 
                          for i in range(0, len(audio_data)-window_size+1, 1600)])
        
        if len(energy) == 0:
            raise ValueError("No valid audio segments found.")

        # Find window with highest energy
        best_window = np.argmax(energy) * 1600
        
        # Extract 1s segment from the best window
        segment = audio_data[best_window:best_window+16000]
